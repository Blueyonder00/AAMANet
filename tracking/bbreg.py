...
class BBRegressor:
    """
    以首帧图像的bbox选取了1000个样本，以conv3的特征进行训练，通过岭回归的方式修正bbox。  """
    ...
    def train(self, X, bbox, gt):
        ...
        self.model.fit(X, Y)  # X:features Y:gtcx-bbcx)/bbw, (gtcy-bbcy)/bbh, log(gtw-bbw), log(gth-bbh)
    ...
    def get_examples(self, bbox, gt):
        bbox[:, :2] = bbox[:, :2] + bbox[:, 2:] / 2     # x, y, w, h->cx, cy, w, h
        gt[:, :2] = gt[:, :2] + gt[:, 2:] / 2           # x, y, w, h->cx, cy, w, h

        dst_xy = (gt[:, :2] - bbox[:, :2]) / bbox[:, 2:]  # (gtcx-bbcx)/bbw, (gtcy-bbcy)/bbh,
        dst_wh = np.log(gt[:, 2:] / bbox[:, 2:])        # log(gtw-bbw),log(gth-bbh)
